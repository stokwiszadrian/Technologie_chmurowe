...
router.get('/:id', async (req, res) => {
    ...
    const movieRows = await client.query("SELECT * FROM movie WHERE id = $1", [id]); 
...
router.post('/', async (req, res) => {
    ...
    const duplicate = await client.query("SELECT * FROM movie WHERE title = $1", [ movieToAdd.title ]);
    ...
    const insertedMovieRows = await client.query(
        ...
        [movieToAdd.title, movieToAdd.genre, movieToAdd.release_date, movieToAdd.description, movieToAdd.image_url, movieToAdd.director ? movieToAdd.director.id : null]
      );
...
router.put('/:id', async (req, res) => {
    ...
    if(movieToAdd.director_id) {
        const director = await client.query("SELECT * FROM person WHERE id = $1", [ movieToAdd.director_id ]);
    ...
    const result = await client.query(`UPDATE movie SET title = $1, genre = $2, release_date = $3, description = $4, image_url = $5, director_id = $6 WHERE id = $7`,
        [movieToAdd.title, movieToAdd.genre, movieToAdd.release_date, movieToAdd.description, movieToAdd.image_url, movieToAdd.director_id ? movieToAdd.director_id : null, id]
    );
